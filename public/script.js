async function convertir() {
	...
	if (unidad === "kg") {
		url = `/convert/kg/${valor}`;
	} else {
		url = `/convert/lb/${valor}`;
	}
	...
	if (unidad === "kg") {
		document.getElementById("resultado").innerHTML = `${data.kg} kg = ${data.lb} lb`;
	} else {
		document.getElementById("resultado").innerHTML = `${data.lb} lb = ${data.kg} kg`;
	}
}